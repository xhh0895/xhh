...
def downLoadMaptile(zoom, xtile, ytile, file_path):
    ...
    try:
        ...
        print('网络错误')
        flag =  False
        with open('D:/maptile/beijing/err.log', 'a+', encoding='utf8') as log:
...
if __name__ == '__main__':
    ...
    for x in range(10, 17):
        ...
        """
        grid1 = deg2num(39.942844, 116.602104, x)
        grid2 = deg2num(39.874899, 116.715401, x)
        x_tile_range = (grid1[0], grid2[0])
        y_tile_range = (grid1[1], grid2[1])

        print(x_tile_range)

        for x_tile in range(min(x_tile_range[0], x_tile_range[1]), max(x_tile_range[0], x_tile_range[1] + 1), 1):
            thread_pool.submit(downLoadTask,zoom_path,x_tile,y_tile_range)
        # for x_tile in range(53950, 54000 ,1):
        #     thread_pool.submit(downLoadTask,zoom_path,x_tile,y_tile_range)
        # print('=========================')
